...
#             S=list_values
#         else:
#             S=-np.nansum(list_values**2 * np.log(list_values**2))
# =============================================================================
       # return S
    return entropy
...
def decompose_wave(dataset, waveletname):
    koef = []
    for signal_no in range(0, len (dataset)):
        signal = dataset[signal_no]
        pywt.wavedec(signal, waveletname, level=4)
        koef.append()
    return signal
'''
Shannon Entropy
...
def get_ppg_features_entropy(dataset, labels, waveletname):
    ...
    for signal_no in range(0, len (dataset)):
        features = []
        signal = dataset[signal_no]
        list_coeff = pywt.wavedec(signal, waveletname, level=4)
        #no=0
        for coeff in list_coeff:
            #if no > 0 :
...
def load_dataPPG1(signal_path, number):
    signal_file = "%s/%s.csv" % (signal_path, number)
    signal = pd.read_csv(signal_file, delimiter= ';', low_memory= False).replace(";",100)
    ppg_signal = np.array(signal.iloc[0:,0])
    np.save('%s.npy'%(number), ppg_signal)
    return ppg_signal
...
def listtocsv1(data, judul):
    np.savetxt(judul, data, delimiter = ',')
    print(judul, 'added')
    return
...
def time_domain_features_stat(signal_array,sample_rate=500 , preprocess=False):
    features = []
    i = 0
    for signal in signal_array:
        if (preprocess):
        ...
        if(math.isnan(rata_ppi) or math.isnan(std_ppi) or math.isnan(rata_sinyal) or math.isnan(std_sinyal)):
          ...
          features.append([rata_ppi,std_ppi,rata_sinyal,std_sinyal,median_ppi,var_ppi,median,var])
        else:
            features.append([rata_ppi,std_ppi,rata_sinyal,std_sinyal,median_ppi,var_ppi,median,var])
        i = i+1
    return features
